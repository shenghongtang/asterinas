...
/// Sorts the devices in the same order as Linux: Linux keeps devices in hash
/// buckets indexed by `major % 255` and prints them bucket by bucket.
///
/// Reference: <https://elixir.bootlin.com/linux/v6.16.5/source/fs/proc/devices.c#L15>.
fn sort_like_linux(devices: &mut [(u16, &'static str)]) {
    devices.sort_by_key(|&(major, _)| (major % 255, major));
}
...
impl ProcFileOps for DevicesFileOps {
    fn read_at(&self, offset: usize, writer: &mut VmWriter) -> Result<usize> {
        ...
        let mut char_devices = char::collect_major_devices();
        sort_like_linux(&mut char_devices);
        ...
        let mut block_devices = aster_block::collect_major_devices();
        sort_like_linux(&mut block_devices);
